...
def analyze_sales_trend(df: pd.DataFrame, date_col: str = "Sale_Date") -> Dict[str, Any]:
    ...
    try:
        ...
        # Monthly aggregation
        monthly_sales = analyze_time_series(daily_sales, "day", "count", aggregation="sum", freq="M")
...
def analyze_gross_profit(
    df: pd.DataFrame, gross_col: str = "Gross_Profit", date_col: str = "Sale_Date"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        if date_col in df.columns:
            ...
            monthly_gross = analyze_time_series(df, date_col, gross_col, aggregation="mean", freq="M")
...
def analyze_lead_sources(
    ...
    try:
        ...
        return {
            ...
            "unknown_percentage": next(
                (item["percentage"] for item in lead_summary if item["source"] == "Unknown"),
                0,
            ),
            "time_trend": time_trend,
...
def analyze_inventory_health(
    df: pd.DataFrame, days_col: str = "DaysInInventory", status_col: str = None
) -> Dict[str, Any]:
    """
...
if __name__ == "__main__":
    ...
    summer_indices = np.where((sales_data["Sale_Date"].dt.month >= 6) & (sales_data["Sale_Date"].dt.month <= 8))[0]
    winter_indices = np.where((sales_data["Sale_Date"].dt.month == 12) | (sales_data["Sale_Date"].dt.month == 1))[0]
