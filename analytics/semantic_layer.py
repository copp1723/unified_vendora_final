...
from .rule_engine import BusinessRuleEngine as AdvancedRuleEngine
from .rule_engine import RuleValidationResult
...
class UserSchemaProfile:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "UserSchemaProfile":
        ...
        return cls(
            ...
            adjustments=[
                SchemaAdjustment.from_dict(adj) if isinstance(adj, dict) else adj for adj in data.get("adjustments", [])
            ],
...
class QueryStatistics:
    ...
    def record_query(
        ...
        self.stats["queries_by_confidence"][confidence] = self.stats["queries_by_confidence"].get(confidence, 0) + 1
        ...
        for term, matches in metadata.get("column_matches", {}).items():
            ...
            for match in matches:
                ...
                if column not in self.stats["term_matches"][term]["columns"]:
                    self.stats["term_matches"][term]["columns"][column] = {
                        "count": 0,
                        "confidence_sum": 0,
                    }
        ...
        for ambiguity in metadata.get("ambiguities", []):
            ...
            for candidate in ambiguity.get("candidates", []):
                ...
                if column not in self.stats["ambiguity_patterns"][term]["candidates"]:
                    self.stats["ambiguity_patterns"][term]["candidates"][column] = {
                        "count": 0,
                        "confidence_sum": 0,
                    }
...
class PrecisionScoringEngine:
    ...
    def predict_precision(
        self, query: str, metadata: Dict[str, Any], stats: QueryStatistics
...
class DynamicSchemaProfileManager:
    ...
    def _apply_adjustments(
        self, base_profile: ExecSchemaProfile, adjustments: List[SchemaAdjustment]
    ) -> ExecSchemaProfile:
            ...
            if not column_found and adj.adjustment_type == "alias":
                for i, col_data in enumerate(profile_data["columns"]):
                    ...
                    for alias in col_data["aliases"]:
                        if adj.value.lower() in alias.lower() or alias.lower() in adj.value.lower():
                            # Add the alias
                            if adj.value not in col_data["aliases"]:
                                col_data["aliases"].append(adj.value)
                                profile_data["columns"][i] = col_data
...
class EnhancedQueryRewriter:
    ...
    def __init__(
        self,
        schema_profile: ExecSchemaProfile,
...
class SemanticQueryProcessor:
    ...
    def _get_user_components(
        self, user_id: str, role: Optional[ExecRole] = None
    ) -> Tuple[ExecSchemaProfile, EnhancedQueryRewriter, QueryPrecisionScorer]:
    ...
    def process_query(self, query: str, user_id: str, role: Optional[ExecRole] = None) -> Dict[str, Any]:
        """
        ...
        (
            ...
            confidence_level,
            precision_details,
        ) = self.precision_engine.predict_precision(query, rewrite_metadata, self.stats)
        ...
        if self.rule_engine:
            ...
            rule_set = self.rule_engine.get_rules_for_role(
                schema_profile.role.value if isinstance(schema_profile.role, ExecRole) else schema_profile.role
            )
        ...
        result = {
            ...
            "confidence_level": confidence_level,
            "schema_profile": {
                "role": schema_profile.role.value if isinstance(schema_profile.role, ExecRole) else schema_profile.role,
    ...
    def get_query_debug_info(self, query_result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        debug_info = {
            ...
            "processing": {
                ...
                "statistical_rewrites": query_result["rewrite_metadata"]
                .get("statistical_rewrites", {})
                .get("rewrites", []),
            },
            "rules": {
                "passed": [
                    rule_id for rule_id, result in query_result["rule_results"].items() if result.get("is_valid", True)
                ],
                "failed": [
                    {
                        "rule_id": rule_id,
